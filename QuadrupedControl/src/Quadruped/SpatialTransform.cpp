...
MathTypes::Mat6 SpatialTransform::GetSpatialForm() const
{
	...
	X.block<3, 3>(3, 0) = -R * VectorToSkewMat(p); // Transformation according to book
	//X.block<3, 3>(3, 0) = VectorToSkewMat(p) * R; // Transformation according to normal homog transformations 
	X.block<3, 3>(3, 3) = R;

	return X;
}

MathTypes::Mat6 SpatialTransform::GetSpatialFormTranspose() const
{
	MathTypes::Mat6 X = MathTypes::Mat6::Zero();

	X.block<3, 3>(0, 0) = R.transpose();
	X.block<3, 3>(0, 3) = -(R * VectorToSkewMat(p)).transpose(); // Transformation according to book
	//X.block<3, 3>(0, 3) = (VectorToSkewMat(p) * R).transpose(); // Transformation according to normal homog transformations 
...
MathTypes::Mat6 SpatialTransform::GetSpatialFormForce() const
{
	MathTypes::Mat6 X = MathTypes::Mat6::Zero();
	
	X.block<3, 3>(0, 0) = R;
	X.block<3, 3>(0, 3) = -R * VectorToSkewMat(p); // Transformation according to book
	//X.block<3, 3>(0, 3) = VectorToSkewMat(p) * R; // Transformation according to normal homog transformations 
	X.block<3, 3>(3, 0) = MathTypes::Mat3::Zero();
	X.block<3, 3>(3, 3) = R;
...
SpatialTransform SpatialTransform::GetInverse() const
{
	MathTypes::Mat3 newR = this->R.transpose();
	SpatialTransform X(newR, -this->R * this->p);

	return X;
}

MathTypes::Mat3 SpatialTransform::GetRotation() const  { return R; }
MathTypes::Vec3  SpatialTransform::GetTranslation() const { return p; }
...
SpatialTransform SpatialTransform::operator*(const SpatialTransform& rhs) const
{
	...
	MathTypes::Mat3 newR = R * rhs.R;
	MathTypes::Vec3 newp = rhs.p + rhs.R.transpose() * p; // According to book
	//MathTypes::Vec3 newp = p + R * rhs. p; // According to homog transformations
